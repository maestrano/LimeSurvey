...
/**
* Map Connec Resource representation to/from LimeSurvey Model
* You need to extend this class an implement the following methods:
* - getId($model) Returns the LimeSurvey entity local id
* - loadModelById($local_id) Loads the LimeSurvey entity by its id
* - mapConnecResourceToModel($resource_hash, $model) Maps the Connec resource to the LimeSurvey entity
* - mapModelToConnecResource($model) Maps the LimeSurvey entity into a Connec resource
* - persistLocalModel($model) Saves the LimeSurvey entity
* - matchLocalModel($resource_hash) (Optional) Returns an LimeSurvey entity matched by attributes
*/
abstract class BaseMapper {
  ...
  // Overwrite me!
  // Map the Connec resource attributes onto the LimeSurvey model
  abstract protected function mapConnecResourceToModel($resource_hash, $model);

  // Overwrite me!
  // Map the LimeSurvey model to a Connec resource hash
  abstract protected function mapModelToConnecResource($model);

  // Overwrite me!
  // Persist the LimeSurvey model
  ...
  // Persist a list of Connec Resources as LimeSurvey Models
  ...
  // Map a Connec Resource to an LimeSurvey Model
  ...
  // Find an LimeSurvey entity matching the Connec resource or initialize a new one
  ...
  // Transform an LimeSurvey Model into a Connec Resource and push it to Connec
